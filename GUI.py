...
def setup_window():
    ...
    checkbox_label.grid(row=3, column=0, columnspan = 2, pady=10, padx=5)
    checkboxes_func = []
    for idx, item in enumerate(items):
        ...
        checkbox.grid(row=3, column=idx+2, pady=10, padx=5)
    update_classes("")
    ...
    core_l.grid(row = 4, column = 0, columnspan= 4, pady=10, padx=5)
    vcmd = (root.register(validate_input), '%P')
    core_entry = tk.Entry(root, textvariable = core_nb, width=10, validate="key", validatecommand=vcmd)
    core_entry.grid(row = 4, column = 4, columnspan= 6, pady=10, padx=5)

    # ! Validation
    def start():
        root.destroy()
        P = vp.Playlist(entry_var.get(), cores=core_nb.get(), model=f"weights/yolov8{model_letter.get()}.pt", watch_classes=classes)
        P.initialise()
        P.play()
        setup_window()
    ...
    def create_bt():
        ...
        val_bt.grid(row = 5, column=0, columnspan=8, pady=10, padx=5)
