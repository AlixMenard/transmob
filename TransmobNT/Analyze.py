...
class Analyser:
    ...
    def frame_processing(self, frame_queue, detection_queue):
        """
        Function responsible for frame processing and detection.
        """
        count = 0
        self.cap = cv2.VideoCapture(self.url)
        ...
        while self.cap.isOpened():
            ...
            detected = self.yolo(frame, classes = self.watch_classes_ids, stream=True, verbose=False, device='cpu')
            ...
            # Pass the frame and detections to the next stage
    ...
    def tracking_and_post_processing(self, frame_queue, detection_queue):
        ...
        """
        c_time = None
        time_last_save = saves = 0

        while True:
            # Retrieve the frame and detection results from the queue
            count, frame = frame_queue.get()
            if frame is None:  # Exit condition
                break
            detection, class_map = detection_queue.get()
                ...
                for l in self.lines:
                    ...
                    if l.inbound(x, y, self.fleet.get(id)):
                        ...
                        if crossed and self.screenshots:
                            self.screen(frame, box.xyxy, id, class_name, c_time, l)
                        color = (255, 0, 0)
    ...
    def process(self):
        """
        Main function that manages multiprocessing for frame processing and tracking.
        """
        ...
        detection_queue = multiprocessing.Queue(maxsize=50)

        # Create processes for frame processing and tracking
        frame_process = multiprocessing.Process(target=self.frame_processing, args=(frame_queue, detection_queue))
        tracking_process = multiprocessing.Process(target=self.tracking_and_post_processing,
    ...
    def screen(self, frame, box, id, class_name, c_time, line):
        ...
        if not c_time:
            c_time = self.strt
        file_name = fr'{self.folder}/product/screens/{str_time(time_5(c_time))}_l{line.id}_{id}_{class_name}.jpg'
        #print(file_name)
